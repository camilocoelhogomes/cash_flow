...
export default function ScenarioTabs({ scenarios, projectid }: Props) {
  ...
  const [selectedScenario, setSelectedScenario] = React.useState<IGetScenarioById>()
  ...
  React.useEffect(() => {
    setStatus('loading')
    projectStore.getScenarioById(projectid, scenarioId).then((value) => { setSelectedScenario(value); setStatus('loaded'); console.log(value) })
  }, [scenarioId])

  return (
    <Tabs.Root defaultValue={scenarios[0].id.toString()} onValueChange={(value) => setScenarioId(Number(value))}>
      <Tabs.List>
      ...
      </Tabs.List>
      {
        ...
        <Tabs.Content value={selectedScenario.id.toString()} >
          <Layout.ScrollArea type="always" scrollbars="vertical" size={'1'} style={{ height: '70vh', padding: '0% 2%' }}>
            ...
            <div className='grid grid-cols-2 gap-10 '>
              <ContextCard title='Áreas' onEditClick={() => setupdateAreas(!updateAreas)}>
                <Fields.Card label='Área Total' value={selectedScenario.areas.totalArea.toString()} />
                <Fields.Card label='Área Protegida' value={selectedScenario.areas.protectedArea.toString()} />
                <Fields.Card label='Área de Vias' value={selectedScenario.areas.streetArea.toString()} />
                <Fields.Card label='Área Verde (Decoração)' value={selectedScenario.areas.decorationArea.toString()} />
                <Fields.Card label='Nº de Lotes' value={selectedScenario.areas.totalSlots.toString()} />
                <Fields.Card label='Área Líquida Vendável' value={slotArea(selectedScenario?.areas)} />
              </ContextCard>

              <ContextCard title='Precificação' onEditClick={() => setupdatePricing(!updatePricing)}>
                <Fields.Card label='Valor do m² (R$)' value={selectedScenario.pricing?.squareAmount.toString()} />
                <Fields.Card label='Taxa de juros' value={selectedScenario.pricing?.fee.toString()} />
                <Fields.Card label='Modelo de Juros' value={selectedScenario.pricing?.feeModel.toString()} />
                <Fields.Card label='Parcelas' value={selectedScenario.pricing?.installments.toString()} />
                <Fields.Card label='Valor de Entrada' value={selectedScenario.pricing?.startAmount.toString()} />
                <Fields.Card label='Indexador' value={selectedScenario.pricing?.feeIndex.toString()} />
              </ContextCard>
          ...
          <UpdateAreas open={updateAreas} setOpen={setupdateAreas} scenario={selectedScenario} />
          <UpdatePricing open={updatePricing} setOpen={setupdatePricing} scenario={selectedScenario} projectid={projectid} />
        </Tabs.Content>
      }
    </Tabs.Root>
  )
...
const slotArea = ({ totalArea, protectedArea, streetArea, decorationArea, totalSlots }: IAreas) => ((totalArea - protectedArea - streetArea - decorationArea) / totalSlots).toFixed(2)